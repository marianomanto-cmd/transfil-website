...
(() => {
  ...
  class ThreeDStage extends HTMLElement {
    constructor() {
      ...
      note.className = 'note';
      // Exposed as a part so a host that prints its own localized hint can
      // hide this one — it is hard-coded English.
      note.setAttribute('part', 'note');
      note.textContent = 'Drag to orbit · scroll to zoom · right-drag to pan';
      root.appendChild(note);
      this._toolbar = document.createElement('div');
      this._toolbar.className = 'toolbar';
      // Exposed so the host page can hide it with ::part(toolbar); the
      // exporters themselves are dynamic imports, so a hidden toolbar
    ...
    /** Read-only handles for pages that drive the scene themselves —
     *  per-frame animation, or projecting HTML labels over the canvas. */
    get camera() { return this._camera; }
    get scene() { return this._scene; }
    get renderer() { return this._renderer; }
    get controls() { return this._controls; }

    /** Run `fn(elapsedSeconds)` before every render. Returns a disposer.
     *  Wraps the render loop once, then just appends to the hook list. */
    addFrameHook(fn) {
      if (!this._hooks) {
        this._hooks = [];
        const base = this._loop;
        const t0 = performance.now();
        this._loop = () => {
          const t = (performance.now() - t0) / 1000;
          for (const h of this._hooks) h(t);
          base();
        };
        if (this._renderer) this._renderer.setAnimationLoop(this._loop);
      }
      this._hooks.push(fn);
      return () => {
